 ...
 */
public class ContentSubsDTWReality implements RoutingDecisionEngine {
    ...
    private static final double WARMUP = 288000;//80 Jam // 2.592.000 30 hari     // 6.912.000 80 hari     // 7776000 90 hari
    private static final double WARM = 216000; //60 Jam
    ...
    public ContentSubsDTWReality(Settings s) {
        waktu = s.getInt(DURATION);
    }

    public ContentSubsDTWReality(ContentSubsDTWReality csd) {
    ...
    public void doExchangeForNewConnection(Connection con, DTNHost peer) {
        ...
        ContentSubsDTWReality pHost = getOtherDecisionEngine(peer); // Objek host yang sedang ditemui
        ...
        // Pertukaran peopleRank
            ...
            for (Map.Entry<DTNHost, List<Tuple<Double, Integer>>> entry : pHost.peopleRank.entrySet()) {
                ...
                if (peopleRank.containsKey(key)) {
                    // Jika sudah ada, catat node-node yang ditemui saja
        ...
        for (Map.Entry<DTNHost, Message> entry : getOtherDecisionEngine(peer).subscriptionList.entrySet()) {
            DTNHost key = entry.getKey(); // Dapatkan kunci DTNHost
            Message value = entry.getValue(); // Dapatkan nilai Message
...
//		double time = cek(thisHost, peer);
        ...
        // menambahkan connection kedalam list
        if (etime - time > 3600) {
            history.add(new Duration(time, etime));
        }
        
        if (SimClock.getTime() >= WARM && SimClock.getTime() <= WARMUP) {
            ContentSubsDTWReality pHost = getOtherDecisionEngine(peer);
    ...
    ContentSubsDTWReality pHost = getOtherDecisionEngine(otherHost);
    if (m.getId().contains("S") || pHost.tombstone.containsKey(m.getId())) {
        ...
        for (Map.Entry<DTNHost, Message> entry : getOtherDecisionEngine(otherHost).subscriptionList.entrySet()) {
            ...
            if (id >= range[0] && id <= range[1]) {
                ContentSubsDTWReality dest = getOtherDecisionEngine(key);
                ...
                if (peerRankTuples != null && !peerRankTuples.isEmpty()) {
                    ...
                    //    System.out.println("rankListPeer: " + rankListPeer);
                    ...
                    //    System.out.println("rankListHost: " + rankListHost);
                    } else {
    ...
    public RoutingDecisionEngine replicate() {
        return new ContentSubsDTWReality(this);
    }

    private ContentSubsDTWReality getOtherDecisionEngine(DTNHost h) {
        MessageRouter otherRouter = h.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";

        return (ContentSubsDTWReality) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }
    ...
    // Loop melalui setiap entri dalam peopleRank
    for (Map.Entry<DTNHost, List<Tuple<Double, Integer>>> entry : peopleRank.entrySet()) {
        // Mendapatkan daftar rankTuple dari entri saat ini
        List<Tuple<Double, Integer>> rankList = entry.getValue();
        // Loop melalui setiap rankTuple dalam rankList
